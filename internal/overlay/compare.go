...
func FormatReport(report *CompareReport) string {
	...
	if len(report.Results) == 0 {
		sb.WriteString("All packages are up-to-date!")
		return sb.String()
	}
	...
	if len(unlisted) > 0 {
		sb.WriteString(formatResultSection(unlisted, "Other Packages", ""))
	}
...
//
// Both tables are the SECTION's, and neither is styled here. A second appearance
// would read as a second verdict, and there is only one: every row of both tables
// is `redundant`, which is exactly what the two notes are there to qualify. What
// a heading looks like is the caller's, and from story 047 the report's.
func formatVerdictSection(sec verdictSection, results []CompareResult) string {
	if sec.split == nil {
		return formatResultSection(results, sec.title, sec.note)
	}
	...
	if len(identical) > 0 {
		sb.WriteString(formatResultSection(identical, sec.split.identicalTitle, sec.split.identicalNote))
	}
	if len(differing) > 0 {
		sb.WriteString(formatResultSection(differing, sec.split.differingTitle, sec.split.differingNote))
	}
	return sb.String()
}
...
func formatStatusSummary(results []CompareResult) string {
	...
	sb.WriteString("\n")
	if outdated > 0 {
		fmt.Fprintf(&sb, "Outdated: %d | ", outdated)
	}
	if synced > 0 {
		fmt.Fprintf(&sb, "Up-to-date: %d | ", synced)
	}
	if other > 0 {
		fmt.Fprintf(&sb, "Other: %d | ", other)
	}
...
//
// It chooses no appearance for any of it (S046-R5.2). The heading and the note
// used to carry the section's colour, and a section that arrives pre-coloured
// cannot be re-rendered plain for a log, re-laid-out for Markdown or exported at
// all — which is why the colour left with the rest of this package's printing.
// `overlay compare`'s whole report moves into the report envelope in story 047,
// and the envelope styles what it renders.
func formatResultSection(results []CompareResult, title, note string) string {
	var sb strings.Builder
	...
	fmt.Fprintf(&sb, "\n%s:\n", title)
	if note != "" {
		// Written as a VALUE, never as a format string, like every other piece of
		// text this report prints.
		sb.WriteString(note + "\n")
	}
	sb.WriteString(formatSectionLine(w, "top"))
	sb.WriteString(formatSectionRow(w,
		"Package", "Category", "Bentoo Version", "Gentoo Version", "Status", "Verdict"))
	...
	for _, r := range results {
		...
		sb.WriteString(formatSectionRow(w, pkg, cat, local, remote, status, verdict))
	}
...
// declaredReason is declaredReasonTail's input reached from the other end: the
// tail a RESULT's own declaration would render as.
//
// It exists so the cap is stated once and can be named once. review_test.go
// asserts that a model's proposal is capped by the SAME mechanism as the
// operator's declared reason rather than by a second one that happens to agree
// today, and an assertion spelled as truncateString(long, patchedReasonCap)
// would be that second copy — green on the day the two mechanisms diverged.
func declaredReason(r CompareResult) string {
	return declaredReasonTail(Finding{Effect: declared(r.PatchedReason)})
}

// formatVerificationFindings renders one line per verification finding, beneath
// the section's table.
//
// It is a RENDERER over compareFindings' values and composes no fact of its own,
// which is what keeps the printed line and the value a caller receives the same
// statement. The findings it is handed are re-established from the section's
// results rather than sliced out of CompareReport.Findings, because a section is
// a subset a caller chose and the report-level list is deliberately not narrowed
// by that choice.
//
// It chooses NO APPEARANCE (S046-R5.2). Every line here used to arrive coloured,
// which is a decision only a terminal can use: the same finding could not then be
// written to a Markdown file, exported as JSON or logged plain. The glyphs stay —
// "⚠ " opens a finding this tool stands behind and "· " a fact it is merely
// restating, and they are structure rather than style.
func formatVerificationFindings(results []CompareResult) string {
	return renderCompareFindings(compareFindings(results))
}

// renderCompareFindings turns findings into the lines they print beneath a
// table.
//
// FindingCompared is skipped: the table above has just printed that row, and
// printing it again as a sentence would double the report. It is in the list for
// the consumers that have no table, which is the whole reason the findings are
// values.
func renderCompareFindings(findings []Finding) string {
	var sb strings.Builder
	// Counted so the caveat below prints once for the whole section, on the same
	// argument that keeps the removal recommendation out of every row: a sentence
	// repeated eight times is a sentence nobody reads the ninth time.
	//
	...
	for _, f := range findings {
		...
		case FindingUndeclaredDivergence:
			fmt.Fprintf(&sb, "⚠ %s: %s\n", f.Atom, f.Detail)
			if f.Authorship != AuthorshipOverlay {
				unproved++
			}
			// The model's reading of this same difference, beneath the finding it
			// is about, for the same reason the caveat is printed below: this is
			// the ambiguous finding, and a model's guess at the direction is
			// exactly what the operator has otherwise to establish by hand. It
			// qualifies the ambiguity — it does not resolve it, which is why the
			// caveat still prints beneath the section.
			sb.WriteString(renderReviewCommentary(f))
		case FindingDeclaredDivergence:
			fmt.Fprintf(&sb, "· %s: %s%s\n", f.Atom, f.Detail, declaredReasonTail(f))
		case FindingCompared:
			// The table said it.
		}
	}

	// The caveat the count above cannot state on its own, printed once and only
	// where it applies.
	//
	...
	//
	// It is NOT itself a Finding, and that is deliberate: it is about a SECTION
	// rather than about a package, so it has no atom to carry, and a finding
	// whose identifier is blank is exactly what R5.1 forbids. A consumer with no
	// sections re-derives it the way this loop does, from the findings it holds.
	if unproved > 0 {
		sb.WriteString(undeclaredDivergenceCaveat + "\n")
	}

	return sb.String()
}

// renderReviewCommentary renders a model's reading of one finding — where the
...
//
// EVERY MODEL-PRODUCED STRING IS WRITTEN AS A VALUE and never as a format
// string, exactly like ProvedBy, the declared reason and every other piece of
// text this report prints. It reaches a terminal and nothing else: no shell, no
// command, no file. Its whitespace was collapsed when the finding was
// established, so a summary cannot forge a second line here.
//
// Nothing here can change a Verdict, a count or which table a package sits in
// (R5.8): it turns one finished Finding into at most two lines.
func renderReviewCommentary(f Finding) string {
	prose := reviewOriginProse(f.Origin)
	if prose == "" || f.Effect.Source != EffectReviewed || f.Effect.Text == "" {
		return ""
	}

	var sb strings.Builder
	fmt.Fprintf(&sb, "%s%s — %s\n", reviewReadingLead, prose, f.Effect.Text)

	declaration := truncateString(f.Proposal, patchedReasonCap)
	if declaration == "" {
		// An overlay-origin note with nothing to propose, or a note from any other
		// origin, which carries no proposal by construction. The reading above
		// still stands; an empty proposal line would read as a truncation bug.
		return sb.String()
	}
	// Capped by the SAME mechanism as the operator's own declared reason
	// (declaredReasonTail, below), not by a second one: an unbounded proposal
	// would let a model's essay decide the width of the whole report. The cap is
	// applied HERE and not on the finding, because a width is a terminal's
	// business and an export has none to respect.
...
// formatSectionRow creates a row of the section table.
//
// The header row and a data row are now the SAME row, and that is what the
// colours leaving cost and paid for. The two used to differ only in what was
// wrapped in an escape sequence — the header in one colour, the two judgement
// cells in theirs — and the separate data-row branch existed solely so a cell
// could be padded BEFORE being coloured, since padding a string that already
// carries escape codes counts them toward its width and misaligns the table.
// With no colour to apply there is one format string, one padding rule, and no
// ordering left to get wrong.
//
// Which row is the header is still information, and it is still carried: the
// caller writes the header row first and draws a rule under it
// (formatSectionLine "mid"). What it LOOKS like is whoever renders it, and from
// story 047 that is the report (S046-R5.2).
func formatSectionRow(w columnWidths, pkg, cat, local, remote, status, verdict string) string {
	return fmt.Sprintf("│ %-*s │ %-*s │ %-*s │ %-*s │ %-*s │ %-*s │\n",
		w.pkg, pkg, w.cat, cat, w.local, local, w.remote, remote, w.status, status, w.verdict, verdict)
}
...
//
// header no longer selects anything: all it ever chose was the header colour,
// and this package chooses no colour (S046-R5.2). The parameter stays because it
// is the narrow table's shape and the shape is what its callers pin — a renderer
// that wants a distinguishable header row still knows which row it is; drawing
// the distinction is just not this package's job.
func formatTableRow(pkgW, catW, localW, remoteW int, pkg, cat, local, remote string, header bool) string {
	_ = header
	return fmt.Sprintf("│ %-*s │ %-*s │ %-*s │ %-*s │\n",
		pkgW, pkg, catW, cat, localW, local, remoteW, remote)
}
...
func FormatSummary(report *CompareReport) string {
	...
	if report.OutdatedCount > 0 {
		fmt.Fprintf(&sb, "Outdated: %d\n", report.OutdatedCount)
	}
	if report.NewerCount > 0 {
		fmt.Fprintf(&sb, "Newer in Bentoo: %d\n", report.NewerCount)
	}
	if report.UpToDateCount > 0 {
		fmt.Fprintf(&sb, "Up-to-date: %d\n", report.UpToDateCount)
	}
